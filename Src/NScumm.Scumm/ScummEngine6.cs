...
namespace NScumm.Scumm
{
    ...
    partial class ScummEngine6: ScummEngine5
    {
        ...
        public ScummEngine6(GameSettings game, 
        ...
        protected override void ResetScummVars()
        {
            ...
            {
                DefineArray(array.Index, array.Type == 0 
                    ? ArrayType.IntArray 
                    : (ArrayType)array.Type, array.Dim2, array.Dim1);
            }
        }
        ...
        {
            var opcodes = (from method 
                           in GetType().GetMethods(BindingFlags.Instance | BindingFlags.NonPublic)
                                    let attributes =
                                    (OpCodeAttribute[])method.GetCustomAttributes
                                                                     (typeof(OpCodeAttribute), false)
            ...
            {
                _opCodes.Add(op.Key, op.OrderByDescending(
                    o => GetDepth(o.DeclaringType)).Select(OpCode).First());
            }
        }
        ...
        Action OpCode(MethodInfo method)
        {
            ...
            { 
                try
                {
                    ...
                    this.Trace().Write(TraceSwitches.OpCodes, 
                        "Room = {1,3}, Script = {0,3}, Offset = {4,4}, Name = [{3:X2}] {2}({5})", 
                        ...
                        => string.Format("{0}={1}", parameterNames[i], GetDebuggerDisplayFor(p)))));
